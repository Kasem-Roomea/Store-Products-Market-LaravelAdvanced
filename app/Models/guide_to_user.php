...
class guide_to_user extends GeneralModel
{
    protected $table = 'guide_to_user';

    public static function createUpdate($params){

        $record= isset($params["id"])? self::find($params["id"]) :new self();
        $record->colName = isset($params["colName"])? $params["colName"]: $record->colName;
        $record->image = isset($params["image"])?helper::base64_image( $params["image"],'guide_to_user'): $record->image;
    ...
    public function ModelName2(){
        return $this->hasMany("\App\Models\ModelName2",'guide_to_user_id');
    }
}