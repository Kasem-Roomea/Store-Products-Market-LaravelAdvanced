...
class social_links extends GeneralModel
{
    protected $table = 'social_links' , $appends=['img'];

    public static function createUpdate($params){

        $record= isset($params["id"])? self::find($params["id"]) :new self();
        $record->colName = isset($params["colName"])? $params["colName"]: $record->colName;
        $record->image = isset($params["image"])?helper::base64_image( $params["image"],'social_links'): $record->image;
    ...
    public function ModelName2(){
        return $this->hasMany("\App\Models\ModelName2",'social_links_id');
    }
