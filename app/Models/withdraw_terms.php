...
class withdraw_terms extends GeneralModel
{
    protected $table = 'withdraw_terms';

    public static function createUpdate($params){

        $record= isset($params["id"])? self::find($params["id"]) :new self();
        $record->colName = isset($params["colName"])? $params["colName"]: $record->colName;
        $record->image = isset($params["image"])?helper::base64_image( $params["image"],'withdraw_terms'): $record->image;
    ...
    public function ModelName2(){
        return $this->hasMany("\App\Models\ModelName2",'withdraw_terms_id');
    }
}